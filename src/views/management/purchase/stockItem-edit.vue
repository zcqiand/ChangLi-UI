<template>
	...
	<el-dialog v-model="dialogVisible" :title="currentUpdateId === undefined ? '新增' : '修改'" @close="resetForm" width="50%">
		<el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="right">
			<el-form-item prop="stockId" label="库存标识">
				<el-input v-model="formData.stockId" placeholder="请输入" />
			</el-form-item>
...
import { getApi, createApi, updateApi } from "@/api/management/erp/stockItem"
...
const handleUpdate = (id: undefined | string) => {
	if (id === undefined) {
		...
		getApi({
			...
			.then((res: any) => {
				formData.stockId = res.data.stockId
...
const formRules: FormRules = reactive({
	stockId: [{ required: true, trigger: "blur", message: "请输入库存标识" }],
