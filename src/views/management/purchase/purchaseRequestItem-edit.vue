<template>
	...
	<el-dialog v-model="dialogVisible" :title="currentUpdateId === undefined ? '新增' : '修改'" @close="resetForm" width="50%">
		<el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="right">
			<el-form-item prop="purchaseRequestId" label="采购需求标识">
				<el-input v-model="formData.purchaseRequestId" placeholder="请输入" />
			</el-form-item>
...
import { getApi, createApi, updateApi } from "@/api/management/erp/purchaseRequestItem"
...
const handleUpdate = (id: undefined | string) => {
	if (id === undefined) {
		...
		getApi({
			...
			.then((res: any) => {
				formData.purchaseRequestId = res.data.purchaseRequestId
...
const formRules: FormRules = reactive({
	purchaseRequestId: [{ required: true, trigger: "blur", message: "请输入采购需求标识" }],
